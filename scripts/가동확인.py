...
## 두 다리로 선다
① GitHub Actions(6시간마다) = **즉시 경보**. 평소엔 조용하고, 안 돈 회차가 생기면
   그 자리에서 문자를 보낸다.
② 사장님 PC 예약작업 = **하루 한 번 확인**. ①이 통째로 멈춘 날에도 소리가 나게.
GitHub 이 멈춘 날은 ②만 살아 있다.

## ★2026-09-03 사장님 확정으로 바뀐 것
사장님 원문: "진짜 실패인가 아니면 그냥 시스템 상 조금 지연된것인가? 실행중이라면
문제가 없음. 근데 실행 자체를 안한거면 그게 진짜 실패" / "몇번 정상이 아니라
비정상 뜨는 즉시 알려달라고."
  · 세는 문구("4번 중 3번 정상")를 없앴다. 정상일 때는 **한 통도 안 보낸다.**
  · 판정을 하루 1회 → 6시간마다로 올려, 안 돈 회차를 최대 21시간이 아니라
    3~9시간 안에 알린다.
  · 매일 오는 살아있음 한 줄은 팀프로젝트 쪽이 맡는다. 여기서는 판정 값만
    실행 요약란에 남기고(결과_한줄) 새 정기 문자를 만들지 않는다.
"""
...
#
#   ★2026-09-03 재조정. 2시간이던 값은 **보류를 만들 수 없게** 만들고 있었다.
#   그때는 판정이 하루 한 번(KST 09:00)뿐이라 네 회차의 '끝난 뒤 경과'가
#   21h / 15h / 9h / 3h 였다. 가장 짧은 3h 도 2h 를 넘으니, 아직 큐에 있는 실행은
#   **언제나** 안 돎으로 찍혔다. 8/7 에 9시간을 기다렸다가 성공한 회차가 그렇게
#   '안 돌았음'으로 보고됐다 — 멀쩡한데 경보가 나간 것이다.
#
#   이제 판정을 6시간마다 한다(가동확인.yml). 같은 회차가 끝난 뒤
#   3h → 9h → 15h → 21h 네 번 판정된다. 그래서 한계를 그 **3h 와 9h 사이**에 둔다:
#     · 첫 판정(3h) — 아직 도는 중이면 봐준다(보류). 문자 안 나간다.
#     · 다음 판정(9h) — 그때도 안 끝났으면 안 돎으로 본다. 그때 문자가 나간다.
#   5시간으로 두는 이유는 GitHub 예약이 늘 몇십 분 늦게 뜨기 때문이다 —
#   아래(3h)로 2시간, 위(9h)로 4시간 여유가 남는다.
#   (이 관계는 글이 아니라 검사가 지킨다: test_가동확인.py 의
#    Test보류_한계가_라이브_판정시각과_맞는가 — 라이브 cron 에서 직접 계산한다.)
보류_한계 = timedelta(hours=5)
...
def 새로_안돎(실행들: list, 회차들: list, 지금: datetime) -> set:
    """이번 판정에서 **처음** 안 돎이 된 회차의 라벨.

    ★왜 필요한가(2026-09-03). 판정을 6시간마다 하면 같은 회차를 네 번 본다
      (끝난 뒤 3h·9h·15h·21h). 그대로 알리면 한 번 빠진 회차 하나로 문자가
      서너 통 간다 — 사장님이 없애라고 하신 그 소음이다.

    같은 실행 기록으로 **직전 판정(6시간 전)을 그대로 다시 계산**해서, 그때 안 돎이
    아니었던 것만 고른다. 상태 파일도 표식도 없이 정확히 한 통이 나간다.
    (상태를 파일에 적으면 그 파일이 사라지는 날 침묵이 된다 — 이 저장소가 이미 겪었다.)

    ★직전이 본 **회차 목록까지** 다시 만든다. 그때는 아직 끝나지도 않은 회차를
      '그때도 안 돎이었다'고 읽으면, 새로 빠진 회차가 영원히 문자로 안 나간다.
    ★맞춰보는 열쇠는 라벨이 아니라 회차 시작 시각이다 — '00~06시' 는 날마다 돌아온다.
    """
    앞판정 = 지금 - 회차_길이
    앞회차 = 최근_끝난_회차(앞판정, 개수=len(회차들))
    직전 = {r["시작"]: r["상태"] for r in 판정(실행들, 앞회차, 앞판정)}
    이번 = 판정(실행들, 회차들, 지금)
    return {r["회차"] for r in 이번
            if r["상태"] == 안돎 and 직전.get(r["시작"]) != 안돎}


def build_report(결과: list, 새것: set = None) -> str:
    """비정상일 때만 문구를 만든다. 이상 없으면 **빈 문자열**(= 문자 안 보냄).

    ★2026-09-03 사장님 확정으로 계약이 바뀌었다.
      원문: "몇번 정상이 아니라 비정상 뜨는 즉시 알려달라고."
      그래서 '4번 중 3번 정상' 같은 세는 문구를 없앴고, 잘 돌았을 때 나가던
      매일 한 줄도 없앴다(그 한 줄은 팀프로젝트 쪽이 맡는다).

    Args:
        결과: 판정() 결과.
        새것: 이번에 처음 안 돎이 된 회차 라벨 집합(새로_안돎). None 이면
              안 돈 회차를 전부 새것으로 본다(PC 쪽 하루 1회 판정용).
    """
    산것 = [r for r in 결과 if r["상태"] == 정상]
    빠짐 = [r for r in 결과 if r["상태"] == 안돎]
    # 정상이 하나도 없는데 빠진 것도 없다 = 전부 아직 도는 중. 조용히 넘기면
    # 러너 기근 날 무음이 된다(2026-08-07 실측 재현).
    if not 산것 and not 빠짐:
        return ("🔴 상노 점검이 계속 시작만 하고 안 끝나고 있습니다"
                "(GitHub 이 일할 컴퓨터를 안 내주는 중일 수 있습니다).\n"
                "→ 시트 숫자가 갱신되지 않고 있습니다. 확인이 필요해요.")
    if not 빠짐:
        return ""
    if len(빠짐) == len(결과):
        # 하루치가 통째로 빈 날은 이미 알렸든 아니든 매번 알린다 — 가장 나쁜 경우다.
        return "🔴 상노 점검이 " + _결측_설명(결과)
    알릴것 = [r for r in 빠짐 if 새것 is None or r["회차"] in 새것]
    if not 알릴것:
        return ""
    return ("🔴 상노 점검이 안 돌았습니다 "
            f"({', '.join(r['회차'] for r in 알릴것)}).\n" + _결측_설명(결과))


def 결과_한줄(결과: list) -> str:
    """판정 값 한 줄. **문자가 아니라 값이다.**

    매일 오는 살아있음 한 줄은 팀프로젝트 쪽이 만든다(2026-09-03 사장님 확정) —
    여기서 새 정기 문자를 만들지 않는다. 대신 그쪽이 읽어갈 값만 남긴다.
    """
    return json.dumps({r["회차"]: r["상태"] for r in 결과}, ensure_ascii=False)
...
def _결과_넘기기(결과: list) -> None:
    """판정 값을 이 실행의 요약란에 남긴다. 문자는 안 보낸다.

    팀프로젝트 쪽이 매일 한 줄(살아있음 증거)을 만들 때 이 값을 읽어간다.
    """
    한줄 = 결과_한줄(결과)
    print(f"[가동확인-결과] {한줄}")
    경로 = os.environ.get("GITHUB_STEP_SUMMARY", "")
    if not 경로:
        return
    try:
        with open(경로, "a", encoding="utf-8") as f:
            f.write(f"[가동확인-결과] {한줄}\n")
    except OSError as e:
        print(f"[가동확인] 요약란 기록 실패(무시): {type(e).__name__}")
...
def main(argv=None) -> int:
    ...
    if a.모드 == "보고":
        _결과_넘기기(결과)
        문구 = build_report(결과, 새로_안돎(실행, 회차들, 지금))
        if not 문구:
            print("[가동확인] 이상 없음 — 문자 없음"
