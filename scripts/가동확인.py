...
#   보류로만 두면 판정 창이 24시간을 겹침 없이 자르며 **영원히 사면**된다
#   — 하루 한 번만 판정하기 때문이다.
#
#   숫자 근거(2026-08-07 정정). 처음엔 12시간이었는데 **재는 지점이 달랐다** —
#   "run 31128005421 이 12시간 queued" 는 *실행 생성* 시점 기준이고, 이 상수는
#   *회차 종료* 시점부터 잰다. 실측(완료 198건, 07-14~08-07)으로 다시 재니
#   회차 종료를 넘겨 생성된 실행은 **0건**이었다.
#   더 결정적인 건, 판정 시각(09:00)에서 네 회차의 '끝난 뒤 경과'가
#   21h / 15h / 9h / **3h** 라는 것이다. 12시간으로 두면 뒤쪽 두 회차
#   (18~00시·00~06시)는 **영원히 승격되지 못한다** — 매일 3·4번째 자리에만 놓이고
#   그 창은 두 번 다시 안 본다. 하필 그 구간이 KST 18~06시 = UTC 09~21시로
#   러너가 가장 붐비는 시간대다.
#   → 가장 짧은 경과(3h)보다 작아야 네 회차 다 승격 가능. 실측 여유 넉넉히 2시간.
보류_한계 = timedelta(hours=2)
...
def build_report(결과: list) -> str:
    ...
    if 빠짐:
        # ★목록과 아래 '몇 시간 전 것' 숫자가 같은 것을 세게 한다.
        #   예전엔 목록은 안돎만, 숫자는 보류까지 세서 사장님이 맞춰볼 수 없었다.
        갱신안됨 = [r for r in 결과 if r["상태"] != 정상]
        줄 += "\n갱신 안 된 점검: " + ", ".join(
            r["회차"] + ("(아직 도는 중)" if r["상태"] == 보류 else "") for r in 갱신안됨)
        줄 += "\n" + _결측_설명(결과)
    return 줄
