...
def test_몇_퍼센트_읽었는지를_표에_적는다():
    ...
    말 = 읽은정도_말({"묶음": 338, "못읽은묶음": 73, "검색확인": 100,
                  "검색막힘": 5, "확정제품": 442})
    assert "댓글 78% 읽음(265/338묶음)" in 말
    # ★한 단계만 재고 표 전체가 온전한 것처럼 말하면 안 된다(2026-09-05 검수).
    assert "이름 확인 95/100" in 말 and "오늘 확정 442종" in 말
...
# ★2026-09-04 — '상위노출 차지' 칸을 폐기했다(독립 검수 지적 #6).
#   그 칸은 `원천 == "상위노출"` 인 언급만 셌는데, 그 값을 넣는 코드가 저장소에
#   **한 줄도 없었다**(candidates_from_title 은 만들어 놓고 한 번도 안 불렸다).
#   그래서 라이브에서 이 칸과 '우리가 놓친' 이 **늘 0** 이었고, 화면은 그 0을
#   "우리가 놓친 것 없음" 으로 보이게 했다.
#   되살리는 대신 없앴다 — 애초에 상위 구좌 글만 훑으므로 '나온 키워드 수' 와
#   언제나 같은 값이다. 같은 것을 두 번 적으면 사장님이 다른 뜻으로 읽으신다.
#   '우리가 놓친' 은 원천을 안 따지고 세도록 고쳐 되살렸다.


def test_우리가_놓친_키워드를_원천_안_따지고_센다():
    mentions = [
        {"표시": "맥단비", "키": "맥단비", "종류": "제품", "댓글": "맥단비 탈모샴푸 후기",
         "키워드": "비듬샴푸", "원천": "댓글", "우리놓침": True},
        {"표시": "맥단비", "키": "맥단비", "종류": "제품", "댓글": "맥단비 샴푸 써봄",
         "키워드": "두피각질", "원천": "댓글", "우리놓침": False},
    ...
    assert len(rows) == 1
    assert rows[0]["횟수"] == 3
    assert rows[0]["놓친"] == 2, "우리 글이 하나도 없던 키워드 수"
    assert "상위노출" not in rows[0], "폐기한 칸이 되살아나면 안 된다"


def test_폐기한_상위노출_차지_칸은_표에_없다():
    table = build_table([], [{**_today_row(), "놓친": 3}], "2026-07-24")
    head, row = table[0], table[1]
    assert "상위노출 차지" not in head
    assert row[head.index("우리가 놓친")] == 3


def test_글_단위_기록은_횟수에_섞이지_않는다():
    """날짜 열·추세의 재료인 '횟수' 는 **댓글 언급만** 센다.

    댓글을 못 연 글도 계정을 세려고 글 단위로 한 줄 남기는데(원천="글"),
    그것까지 횟수에 넣으면 아무 일도 없었는데 "▲ 늘었다" 가 된다.
    """
    mentions = [
        {"표시": "맥단비", "키": "맥단비", "종류": "제품", "댓글": "맥단비 써요",
         "키워드": "비듬샴푸", "원천": "댓글", "우리놓침": False},
        {"표시": "맥단비", "키": "맥단비", "종류": "제품", "댓글": "",
         "키워드": "두피각질", "원천": "글", "우리놓침": True},
    ]
    rows = confirmed_rows(mentions, {"맥단비": {"제품": True, "이름": "맥단비"}})
    assert rows[0]["횟수"] == 1
    assert rows[0]["놓친"] == 1, "글 단위 기록도 '놓친' 에는 들어간다"


def test_오늘_어느_키워드에서든_보였으면_표에_남는다():
    """7일 댓글 합계가 0이어도 오늘 보였으면 내리지 않는다 —
    사장님이 보자고 한 게 바로 그 경쟁사다."""
    row = {"제품군": "샴푸", "경쟁사": "맥단비", "횟수": 0, "놓친": 2,
           "키워드수": 3, "키워드들": ["비듬샴푸"], "글들": [], "댓글 예시": ""}
    table = build_table([], [row], "2026-07-24")
    assert len(table) == 2
    head, out = table[0], table[1]
    assert out[head.index("우리가 놓친")] == 2
