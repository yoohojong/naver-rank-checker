...
def test_닫은_문도_한참_뒤엔_한_번_다시_본다():
    ...
    assert L.무료문_다시볼때인가(지금=1000.0 + L.무료_다시두드리기_초 - 1) is False
    assert L.무료문_다시볼때인가(지금=1000.0 + L.무료_다시두드리기_초) is True
    assert L.무료가_닫혔나() is False
...
def test_묻기만_해서는_문이_안_열린다():
    """★검수 심각 4: 묻는 순간 문이 열려서, 사유를 남길 때는 '닫힘' 이 아니게 됐다.
    그래서 '무료가 한도로 닫혔다' 는 사유가 통째로 사라지고, 부르는 쪽이
    '기다리면 풀림' 을 '고장' 으로 세어 회차를 접었다(9/4 에 고친 사고의 재발).
    """
    L._무료문.update({"닫힘": True, "닫은때": 1000.0, "연속한도": 3})
    한참뒤 = 1000.0 + L.무료_다시두드리기_초 + 10
    # 몇 번을 물어도 상태가 안 바뀐다
    for _ in range(5):
        assert L.무료가_닫혔나(지금=한참뒤) is True
    assert L._무료문["닫힘"] is True


def test_결정과_보고_사이에_사유가_사라지지_않는다(monkeypatch):
    """결정 자리와 보고 자리 사이에 300초 선을 넘어도 사유는 그대로 남아야 한다."""
    _한도만_돌려준다(monkeypatch)
    for _ in range(L.무료_포기_횟수):
        L._groq_call("계", "글", max_tokens=10, timeout=1, sleep=lambda s: None)
    assert L.무료가_닫혔나() is True
    사유 = []
    L._groq_call("계", "글", max_tokens=10, timeout=1, sleep=lambda s: None, errors=사유)
    # 이 뒤에 시간이 아무리 흘러도, 남긴 사유는 '한도로 닫힘' 그대로다
    assert L.무료_건너뜀_말 in 사유
    assert B.한도인가(사유 + ["유료:HTTP 529 — overloaded"]) is False
    assert any(L.무료_건너뜀_말 in x for x in 사유)
