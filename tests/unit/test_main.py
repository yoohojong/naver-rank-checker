...
class TestCompetitorCollectionWiring:
    """경쟁사 수집 배선 (2026-07-23) — 꺼져 있을 때 순위 검사 결과가 한 글자도 안 바뀌어야 한다."""

    def _make_row(self, keyword: str, link: str) -> dict:
        return {"키워드": keyword, "링크": link, "유형": "", "_row": 2, "_tab": "샴푸 카외"}

    def _mock_result(self):
        result = MagicMock()
        result.exposure_area.value = "AB"
        result.parser_confidence = 0.9
        result.block_order = ["AB"]
        result.integrated_rank = 3
        result.cafe_slot_rank = 1
        result.in_jisikin = False
        result.matched_url = None
        return result

    def test_off_path_returns_identical_columns(self):
        """수집기 없음(기본) = 있음(None 전달) = 결과 동일 + 수집 함수 호출 X."""
        from src.main import _process_row
        from src.health import HealthMonitor

        crawler = MagicMock()
        crawler.fetch_search.return_value = "<html>" + "x" * 600 + "</html>"
        link = "https://cafe.naver.com/cosmania/12345"

        with patch("src.main.parse_search_result", return_value=self._mock_result()), \
             patch("src.main.collect_slot_items") as collect:
            base = _process_row(self._make_row("샴푸", link), crawler, HealthMonitor())
            off = _process_row(
                self._make_row("샴푸", link), crawler, HealthMonitor(),
                competitor_collector=None, competitor_date="2026-07-23",
            )

        assert base == off
        collect.assert_not_called()  # 꺼져 있으면 HTML 을 한 번도 더 읽지 않는다

    def test_on_path_collects_without_changing_columns(self):
        """켜져 있어도 순위 검사 결과 컬럼은 그대로 — 수집만 추가된다."""
        from src.main import _process_row
        from src.health import HealthMonitor
        ...
        crawler.fetch_search.return_value = "<html>" + "x" * 600 + "</html>"
        link = "https://cafe.naver.com/cosmania/12345"
        collector = CompetitorCollector(our_cafe_slugs={"cosmania"})

        fake_items = [SimpleNamespace(
            area="인기글", rank=1, url="https://cafe.naver.com/rival/1",
            kind="cafe", title="라이벌 글", block_name="인기글", source_name="라이벌카페",
        )]

        with patch("src.main.parse_search_result", return_value=self._mock_result()), \
             patch("src.main.collect_slot_items", return_value=fake_items):
            base = _process_row(self._make_row("샴푸", link), crawler, HealthMonitor())
            on = _process_row(
                self._make_row("샴푸", link), crawler, HealthMonitor(),
                competitor_collector=collector, competitor_date="2026-07-23",
            )

        assert base == on
        assert len(collector) == 1
        assert collector.rows()[0]["주체"] == "rival"

    def test_collection_failure_does_not_break_rank_check(self):
        """수집이 터져도 순위 검사 결과는 그대로 나와야 한다."""
        from src.main import _process_row
        from src.health import HealthMonitor
        from src.competitor import CompetitorCollector
        ...
        with patch("src.main.parse_search_result", return_value=self._mock_result()), \
             patch("src.main.collect_slot_items", side_effect=RuntimeError("boom")):
            base = _process_row(self._make_row("샴푸", link), crawler, HealthMonitor())
            broken = _process_row(
                self._make_row("샴푸", link), crawler, HealthMonitor(),
                competitor_collector=CompetitorCollector(), competitor_date="2026-07-23",
            )
