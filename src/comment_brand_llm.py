...
_SYSTEM = (
    ...
    "   · 증상·성분·약 일반명: 지루성, 비듬, 어성초, 카페인, 맥주효모, 살리실산,\n"
    "     케라틴, 호호바, 티트리, 징크피리치온, 유황, 칼라민\n"
    "     (판테놀·큐텐처럼 제품명으로도 팔리는 애매한 것은 함부로 false 로 두지 마라)\n"
...
def 무료가_닫혔나(지금=None) -> bool:
    """지금 무료 문이 닫혀 있나 — **묻기만 한다. 상태를 바꾸지 않는다.**

    ★2026-09-05 검수 심각 4: 전에는 묻는 순간 문을 열어 버렸다. 그런데 이 함수는
    한 묶음마다 **두 번** 불린다(건너뛸지 정할 때 · 사유를 남길 때). 그 사이가
    유료 시한만큼 길어서, 결정 자리에서는 '닫힘'인데 보고 자리에서는 '열림'이 되어
    **'무료가 한도로 닫혔다' 는 사유가 통째로 사라졌다.** 그러면 부르는 쪽이
    '기다리면 풀림' 을 '고장' 으로 세고 회차를 접는다 — 9/4 에 고친 그 사고다.
    """
    return bool(_무료문["닫힘"])


def 무료문_다시볼때인가(지금=None) -> bool:
    """닫은 지 오래됐으면 **딱 한 번 다시 두드려 보게** 문을 연다 → 열었나.

    ★상태를 바꾸는 것은 이 함수뿐이고, **결정하는 자리에서만** 부른다.
