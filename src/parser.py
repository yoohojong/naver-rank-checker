...
def _owner_runs(urls: list[str]) -> list[list[str]]:
    ...
    실측 근거와 그 한계(2026-08-05 독립검증 M-4 지적 반영):
      ...
      **틀리는 조건이 실재한다(2026-08-05 라이브 확인)**: '닭살피부바디워시' 검색에서
      같은 카페(com2usbaseball2015)가 이웃한 **별도 칸 2개**를 차지했다 — DOM 7칸 대
      출처묶기 6칸. 이때는 DOM 쪽이 맞다. 그래서 DOM 을 읽을 수 있으면 언제나 DOM 이
      우선이고, 이 계산은 DOM 이 죽었을 때만 쓰는 차선이다(그 상황이 이 모양과 겹치면
      순위가 한 칸 당겨질 수 있다 — 알고 감수하는 위험).
    """
    runs: list[list[str]] = []
    for url in urls:
