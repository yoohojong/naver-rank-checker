...
# 이 말 하나만 키워드로 오면 버린다 — 뭉뚱한 장소·일반명이라 검색 후보로 못 쓴다.
# ★프롬프트로도 거르지만(규칙 2) LLM 이 새어 보내는 것을 코드가 한 번 더 막는다
#   (2026-09-06 실물 미리보기에서 '병원' 하나가 새어 나옴). 붙은 말(예: '두피염 병원')은
#   버리지 않는다 — 딱 이 낱말 하나일 때만 버린다.
_뭉뚱한_한낱말 = {
    "병원", "약국", "피부과", "대학병원", "한의원", "의원", "후기", "추천", "가격",
    "제품", "효과", "방법", "질문", "고민", "정보", "구매", "사용", "성분",
}
...
_SYSTEM = (
    ...
    "   · 감탄사·이모지: ㅠㅠ, ㅋㅋ, 헐, !!!\n"
    "   · 뭉뚱한 장소·일반명 하나: 병원, 약국, 피부과, 후기, 추천, 가격, 제품\n"
    "     (증상·부위·제품군과 붙어야 키워드다 — '지루성 두피염 병원'은 되지만 '병원'만은 안 된다)\n"
    ...
    for r in rows:
        ...
        for k in (r.get("키워드") or []):
            ...
            if not k or k in kws:
                continue
            if k in _뭉뚱한_한낱말:          # '병원' 하나만 온 것 — 버린다(붙은 말은 통과)
                continue
            kws.append(k)
        out[t] = kws[:최대_키워드]
    return out
