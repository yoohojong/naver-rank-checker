...
# ★2026-09-06 사장님 교정: "병원은 키워드 후보가 맞지" — 병원·약국·후기·추천 같은 말은
#   함부로 버리면 안 된다(사람들이 실제로 그렇게 검색한다). 낱말 거르기(불용어)를 뒀다가
#   되돌렸다. 게시판 이름·조사 조각만 버리고(그건 사장님도 잡음이라 하심), 나머지는 남긴다.
#   확실하지 않으면 빼지 않는다 — 검색량·사장님 검토가 뒤에서 거른다(원칙 5-1).
...
_SYSTEM = (
    ...
    "   · 감탄사·이모지: ㅠㅠ, ㅋㅋ, 헐, !!!\n"
    "   ※ 병원·약국·후기·추천 같은 말은 버리지 마라 — 사람들이 실제로 그렇게 검색한다.\n"
