...
def build_actor_aliases(history_rows: list[dict]) -> dict:
    ...
    for actors in by_name.values():
        ...
        # 이름 형태가 정확히 하나일 때만 대표로 삼는다(둘 이상이면 어느 쪽인지 알 수 없음).
        if not numeric or len(named) != 1:
            continue
        for actor in numeric:
            aliases[actor] = named[0]
    return aliases
    ...
    *,
    ...
    try:
        ...
        if not created:
            ...
            pruned, span = _delete_row_numbers(ws, stale_rows)
            if span is not None:
                # 통째로 지운 구간 안에 살아남아야 할 행 = 다시 넣는다(순서는 무관).
                readd = [v for num, v in kept_by_row.items() if span[0] <= num <= span[1]]
            kept = list(kept_by_row.values())
        payload = readd + list(rows)
        if payload:
            ws.append_rows(payload, value_input_option="RAW", insert_data_option="INSERT_ROWS")
