...
class SheetsClient:
    ...
    def clear_refresh_flags(self, tab_name: str, row_numbers: list[int]) -> int:
        ...
        (그래야 다음 실행에서 또 재수집되지 않음). '자료조사' 칸은 write_collect_status 로 갱신.
        ...
        headers = ws.row_values(1)
        mapping = map_headers_to_columns(headers)
        if HEADER_REFRESH not in mapping:
            print(
                f"  [REFRESH-CLEAR] {tab_name}: '{HEADER_REFRESH}' 칸 없음 — clear skip. "
                f"재수집은 정상 진행됨."
            )
            return 0

        refresh_col = mapping[HEADER_REFRESH] + 1
        cells = []
        for row_num in sorted({int(r) for r in row_numbers if int(r) >= 2}):
            cells.append({
                "range": gspread.utils.rowcol_to_a1(row_num, refresh_col),
                "values": [[""]],
            })
        if cells:
            _sheets_api_retry(
                lambda: ws.batch_update(cells, value_input_option="RAW"),
                ctx=f"{tab_name} (refresh flag clear)",
            )
        return len(cells)
