...
_MAX_DWELL_LAG = 60  # 코호트 생존곡선 최대 관측 lag(연도 wrap 아티팩트 차단 겸)
...
def _avg_dwell_cohort(backups_by_date: dict) -> tuple:
    ...
    상위노출인 비율 S(L)을 lag 별로 풀링해 합산(관측 가능한 lag 만). 구식 _avg_dwell(연속 spell
    평균)은 누락↔회복 깜빡임에 spell 이 짧게 끊겨 W 를 과소평가 → 코호트 생존곡선으로 교체.
