...
**남은 것(시트 복구, 라이브 키 필요)**: ① 지식인 헤더 'cron 갱신:...' → '지식인탭' 복원(헤더 복원 후 다음 cron 의 formula setup 이 O 공식·값 자동 재생성) ② 보관함 옆 잔여 'cron 갱신:' 열 정리/위치 ③ 보관함 녹색. ⚠️ 서비스계정 키가 GitHub 에만 있어 로컬 직접수정 불가 + 라이브 시트 구조 변경은 사고 이력(D-047 등) 있어 신중 — 사장님 수동 안내 또는 백업 후 Actions 복구 중 택1. (사장님 수동 처리 완료 보고: O 채워짐 확인.)

### D-059: Q&A 봇 '진짜 똑똑하게' — AI(Groq)가 데이터로 답 직접 작성

**배경**: 사장님 "답변이 아직 똑똑하지 못하다". 진단(실증): AI(Groq)는 정상 작동하나, 구조가 **'질문→8개 의도 중 택1→고정 템플릿'**이라 AI 는 라우팅만 하고 답은 정해진 양식. 자유·세밀·복합 질문("샴푸만", "1등 키워드", "좋아진 제품")은 뭉뚱그린/엉뚱한 답. = 설계 한계.
**결정(사장님 '똑똑하게' 선택)**: AI 가 **데이터를 직접 읽고 답을 작성**. 트레이드오프(데이터 외부전송)는 사장님 명시 동의. 전체 시트 대신 **압축 요약**만 전송(토큰·노출 최소).
**구현**: ① `llm_intent.groq_chat` 공통 함수 추출(분류+답작성 공유, 429/5xx 1회 재시도). ② `qa_context.build_context`(제품별 집계 + 카테고리별 키워드 예시 cap=12, raw_* 우선). ③ `llm_answer.compose`(system: 데이터근거·추측금지·인젝션무시·한국어, max_tokens 400). ④ `telegram_qa_bot.answer()` = help 후 **1순위 compose**, 실패/키없음 시 기존 템플릿 폴백(비차단 3단).
**프라이버시**: LLM 에 가는 것 = 제품별 집계 + 키워드 예시(상한)뿐. 링크/원본행/서비스계정/토큰 미전송(회귀 테스트로 보장). owner 전용 응답 + 출력은 텍스트만(코드실행/시트write 없음).
**이중 리뷰**: code-reviewer = HIGH 1(재시도 Request 재사용→매 시도 재생성) + MEDIUM(프롬프트추출 방어→system 가드) 반영. Codex = **NO BLOCKING**.
**검증**: 신규 7 테스트(qa_context 구조·cap·미유출 + compose 성공/실패/메시지), 전체 `pytest -q` = **594 passed**. **실제 Groq 라이브 4질문 정답**: "지식인 구좌?"→샴푸110·바디64·두드178 / "두드러기 1~3등?"→실제 키워드 / "샴푸 상위노출+최고키워드?" / "장사 잘돼?". 버스트(10초 5연속)는 TPM 한도라 컨텍스트 축소(cap20→12)+max_tokens 400+재시도로 완화.
**비용**: 질문당 Groq 1회(~3-4K 토큰), 무료 한도(1000 RPD/12K TPM) 내. 평상시 1건씩이라 무난.

### D-060: 무료 봇 '이해' 최대화 — 도메인 브리핑 + 대화 기억 (Claude 격상은 보류)

**배경**: 사장님 "무료 봇을 너만큼으로 만들면 되지 않냐". **정직한 한계 고지**: 무료 봇 두뇌=Groq Llama(오픈소스), Claude 수준 '지능'은 Claude(유료)만 가능 — 공짜 우회 없음. 단 사장님 진짜 불만은 '지능'보다 '맥락·기억'(이해)이었고, 그건 무료로 크게 개선 가능. (풀 Claude 봇 = 서버0+카드등록+월 Haiku4~7천/Sonnet1.1~1.8만/Opus1.8~3만원 — claude-api skill 단가 산정. 사장님 카드 거부 → 보류, 무료 최대화 우선.)
**구현(무료)**: ① `llm_answer._SYSTEM` 도메인 브리핑 — 사장님 사업(카페글→통합검색 상위노출→외부 신규고객)·제품(카외)·용어(노출영역 AB/인기글/누락/삭제/미노출/재검사필요, 통합순위, 카페구좌순위, 지식인 구좌, 유형)·데이터 읽는 법·사장님 관점을 봇 머리에 심음. ② 대화 기억 — `telegram_qa_bot._history`(최근 6턴) → `llm_answer.compose(history=)` 로 전달 → "그중에/아까 그거" 이어지는 질문 이해.
**검증**: 신규 테스트(compose history + 봇 memory 누적), 전체 `pytest -q` = **635 passed**. **실 Groq 2턴 대화 검증**: Q1"지식인 구좌?"→352 / Q2"그중에 제일 많은 제품?"→"두드러기 178"(기억 적용✅) / Q3"삭제된 거?"→"없다"(도메인 이해✅).
**남은 것**: 사장님이 이 무료 버전 써보고 부족하면 그때 풀 Claude(유료) 격상 재논의. 프라이버시: 질문+데이터요약+최근대화만 Groq 전송(전체 시트 X).

### D-061: 상노 자가복구 — 실패 시 자동 1회 재시도 (코드 자동수정은 안전상 제외)

**배경**: 사장님 "이행 안되면 스스로 왜 안됐는지 파악·수정·재실행 자동화 가능?" + 'cron 종료 수집0/스킵828' 보고 상노 고장 의심. 진단: 그 메시지는 상노 아니라 **별개 신기능 `integration_runner`(카페외부 글감 재료수집, feat/jisikin-kin-api 브랜치서 merge)** — 키워드 '수집 단계' 미태깅 + APIFY 키 미등록이라 전 행 안전 스킵(설계대로). **상노(rank-check)는 정상**(최근 run 전부 success, 0시 run 실제 크롤 진행 중, 봇 순위답 정확).
